...
from protonCoin.db.models.gift import GiftQuery, Gift
from protonCoin.db.models.order import OrderQuery
from protonCoin.db.models.transaction import TransactionQuery

catalog = Blueprint(
    "catalog", __name__, template_folder="templates", static_folder="static"
)
...
def buy():
    ...
    if current_user.balance.amount - gift.price >= 0:
        ...
        flask.flash(f"Заказ на подарок создан, его номер №{order.id}", "success")
    else:
