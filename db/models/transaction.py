...
class TransactionQuery:
    ...
    def get_api(start: int = 0, length: int = 10, search: str | None = None, order_expr=None) -> (
        ...
        transaction_query = transaction_query.join(from_balance_alias, Transaction.from_balance)
        transaction_query = transaction_query.join(to_balance_alias, Transaction.to_balance)
        transaction_query = transaction_query.join(from_user_alias, from_user_alias.id == from_balance_alias.user_id, isouter=True)
        transaction_query = transaction_query.join(to_user_alias, to_user_alias.id == to_balance_alias.user_id, isouter=True)
        count = transaction_query.count()
        if search:
            from_name = from_user_alias.surname + ' ' + from_user_alias.name + ' ' + from_user_alias.patronymic
            to_name = to_user_alias.surname + ' ' + to_user_alias.name + ' ' + to_user_alias.patronymic
            transaction_query = transaction_query.filter(
                or_(from_name.ilike(f'%{search}%'), to_name.ilike(f'%{search}%'), Transaction.comment.ilike(f'%{search}%'),))
            count = transaction_query.count()
