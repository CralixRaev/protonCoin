...
from protonCoin.blueprints.landing.account.forms.achievement import AchievementForm
from protonCoin.blueprints.landing.account.forms.avatar import AvatarForm
from protonCoin.blueprints.landing.account.forms.main import UserForm
from protonCoin.blueprints.landing.account.forms.password import PasswordForm
from protonCoin.db.models.achievement import AchievementQuery
...
from protonCoin.uploads import avatars, achievement_files

from protonCoin.util import password_check, save_upload, upload_avatar
...
def index():
    ...
    if form_avatar.validate_on_submit():
        ...
        error = _check_password(
            form_password.old_password.data,
            form_password.password.data,
            form_password.confirm.data,
        )
        if error:
        ...
        if nickname is not None and nickname != current_user.nickname:
            try:
                ...
                flask.flash(
                    "Никнейм слишком длинный (его длина не должна быть больше 16 символов)",
                    "danger",
                )
                return redirect(url_for(".index"))
...
def declare_achievement():
    ...
    if form.validate_on_submit():
        if not CriteriaQuery.get_criteria_by_id(
            form.criteria_id.data
        ).is_user_achievable:
            flask.flash(
                "Упс... Это начислят автоматически - без твоего участия! ✨🔮",
                "warning",
            )
        else:
            ...
            AchievementQuery.create_achievement(
                form.criteria_id.data,
                current_user.id,
            ...
            flask.flash(
                "Достижение принято. Жди одобрения классным руководителем ⌛", "success"
            )
