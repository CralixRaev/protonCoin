...
from protonCoin.blueprints.manage.admin.gifts.forms.gift import GiftForm
from protonCoin.db.models.gift import GiftQuery
from protonCoin.uploads import gift_images
from protonCoin.util import admin_required, save_upload

gifts = Blueprint(
    "gifts", __name__, template_folder="templates", static_folder="static"
)
...
def create_gift():
    form = GiftForm()
    context = {"title": "Создать подарок", "form": form}
    if form.validate_on_submit():
        image = form.image.data
        filename = save_upload(image, gift_images)
        GiftQuery.create_gift(
            form.name.data,
            form.description.data,
            form.price.data,
            filename,
            form.promo_price.data,
        )
        flask.flash("Подарок успешно создан", "success")
...
def edit_gift():
    ...
    form = GiftForm()
    context = {"title": "Редактировать подарок", "form": form}
    if form.validate_on_submit():
        filename = None
        image = form.image
        if image.data:
            filename = save_upload(image.data, gift_images)
        gift = GiftQuery.update_gift(
            gift,
            form.name.data,
            form.description.data,
            form.price.data,
        ...
        flask.flash("Подарок успешно обновлен", "success")
        return redirect(url_for("manage.admin.gifts.index"))
