...
from protonCoin.db.models.basis import Basis
from protonCoin.db.models.criteria import Criteria
from protonCoin.db.models.group import Group
from protonCoin.db.models.user import User
from protonCoin.uploads import achievement_files
...
class Achievement(db.Model):
    ...
    status = db.Column(
        db.Enum(AchievementStatusEnum),
        default=AchievementStatusEnum.awaiting_approval,
        nullable=False,
    )
    disapproval_reason = db.Column(db.String(4096), nullable=True, default=None)
    approved_disapproved_by = db.Column(
        db.Integer, db.ForeignKey("user.id"), nullable=True, default=None
    )

    user = db.relation(
        "User", foreign_keys=[user_id], backref=backref("achievements", order_by=id)
    )
    approved_disapproved_user = db.relation(
        "User", foreign_keys=[approved_disapproved_by]
    )
...
class AchievementQuery:
    ...
    def create_achievement(
        criteria_id: int, user_id: int, achievement_file: str, comment: str
    ) -> Achievement:
        db.session.rollback()
    ...
    def total_count(is_teacher: bool = False) -> int:
        if is_teacher:
            ...
            return (
                Achievement.query.join(user_alias, user_alias.id == Achievement.user_id)
                .filter(user_alias.group_id == current_user.group_id)
                .count()
            )
        else:
    ...
    def get_api(
        ...
        achievement_query = achievement_query.join(
            user_alias, user_alias.id == Achievement.user_id
        )
        achievement_query = achievement_query.join(
            user_group_alias, user_group_alias.id == user_alias.group_id
        )
        achievement_query = achievement_query.join(
            approved_by_user_alias,
            approved_by_user_alias.id == Achievement.approved_disapproved_by,
            isouter=True,
        )  # we need outer here because some achievements have approved_by user = none
        achievement_query = achievement_query.join(
            criteria_alias, criteria_alias.id == Achievement.criteria_id
        )
        achievement_query = achievement_query.join(
            basis_alias, basis_alias.id == criteria_alias.basis_id
        )
        if is_teacher:
            achievement_query = achievement_query.filter(
                user_group_alias.id == current_user.group_id
            )
        if status:
        ...
        if search:
            user_full_name = (
                user_alias.surname + " " + user_alias.name + " " + user_alias.patronymic
            )
            achievement_query = achievement_query.filter(
                or_(
                    user_full_name.ilike(f"%{search}%"),
                    criteria_alias.name.ilike(f"%{search}%"),
                    basis_alias.name.ilike(f"%{search}%"),
                    Achievement.comment.ilike(f"%{search}%"),
                )
            )
            count = achievement_query.count()
    ...
    def get_achievements(group: Group = None) -> tuple[int, list[Achievement]]:
        achievements_query = Achievement.query.filter(
            Achievement.is_approved is False, Achievement.is_disapproved is False
        )
        if group:
        ...
        return achievements_query.count(), achievements_query.order_by(
            Achievement.id.desc()
        ).all()
    ...
    def get_achievements_approved_disapproved() -> list[Achievement]:
        return (
            Achievement.query.filter(
                or_(
                    and_(
                        Achievement.is_approved is True,
                        Achievement.is_disapproved is False,
                    ),
                    and_(
                        Achievement.is_approved is False,
                        Achievement.is_disapproved is True,
                    ),
                )
            )
    ...
    def get_achievements_by_user(user: User) -> Achievement:
        return (
            Achievement.query.filter(Achievement.user_id == user.id)
            .order_by(Achievement.id.desc())
            .all()
        )
